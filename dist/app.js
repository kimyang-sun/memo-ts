...
var App = (function () {
  function App(appRoot, popUpRoot) {
    ...
    this.bindElementToPopUp('#add_image', MediaInput, function (input) {
      return new ImageItem(input.title, input.url);
    });
    this.bindElementToPopUp('#add_video', MediaInput, function (input) {
      return new VideoItem(input.title, input.url);
    });
    this.bindElementToPopUp('#add_note', TextInput, function (input) {
      return new NoteItem(input.title, input.content);
    });
    this.items.addChild(
      new NoteItem(
        '타입스크립트 객체지향',
    ...
    this.items.addChild(
      new VideoItem('TypeScript Tutorial', 'https://youtu.be/BwuLxPH8IDs')
    );
  }
  App.prototype.bindElementToPopUp = function (
    selector,
    InputType,
    makeSection
  ) {
